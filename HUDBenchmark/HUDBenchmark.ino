...
// Now setup for benchmark / testing 
#ifdef ESP32
#undef F
#define F(s) (s)
#endif
...
// Everything after here is test functions that aren't important
...
int32_t testFilledCircles(uint8_t radius) {
  ...
  for (x = radius; x < gfx->width(); x += r2) {
    for (y = radius; y < gfx->height(); y += r2) {
